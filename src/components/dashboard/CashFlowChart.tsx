...
function TooltipConteudo({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) {
...
export function CashFlowChart({ dados }: { dados: PontoFluxoCaixa[] }) {
  return (
    <ResponsiveContainer width="100%" height={340}>
      <ComposedChart data={dados} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
        ...
        <Legend
          iconType="circle"
          wrapperStyle={{ fontSize: 13, color: '#475569', paddingTop: 12 }}
        />
        <Bar dataKey="receitas" name="Recebimentos" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={28} />
        <Bar dataKey="despesas" name="Pagamentos" fill="#ef4444" radius={[4, 4, 0, 0]} maxBarSize={28} />
        <Line
          type="monotone"
          dataKey="saldo"
          name="Saldo"
