...
export function CompanyRecebidoPagoChart({ empresas }: { empresas: CompanyMetrics[] }) {
  const dados = empresas.map((empresa) => ({
    nome: empresa.shortName,
    Recebido: empresa.recebido,
    Pago: empresa.pago,
  }));
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={dados} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
        ...
        <Bar dataKey="Recebido" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={44} />
        <Bar dataKey="Pago" fill="#ef4444" radius={[4, 4, 0, 0]} maxBarSize={44} />
      </BarChart>
    </ResponsiveContainer>
  );
}
