...
interface PercentFieldProps {
  ...
  hint?: string;
  /** Valor em R$ já calculado (percentual × base) — exibido abaixo do campo para mostrar quanto esse percentual representa. */
  valorCalculado?: number;
}

/** Campo de percentual: guarda fração (0-1), exibe/edita em pontos percentuais (0-100). */
export function PercentField({ label, value, onChange, hint, valorCalculado }: PercentFieldProps) {
  return (
    <label className="flex flex-col gap-1 text-xs font-medium text-graphite-500">
      ...
      {valorCalculado !== undefined && <span className="text-[11px] font-semibold text-graphite-600">{formatCurrency(valorCalculado)}</span>}
